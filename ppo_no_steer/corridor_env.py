...
Version avec contrôle DIRECT des 4 roues indépendantes (pas de volant).
...
class CorridorEnv(gym.Env):
    ...
    CONTRÔLE DIRECT DES 4 ROUES : vitesse angulaire de chaque roue indépendamment.
    ...
    Action (4 valeurs):
        - wheel_fl: vitesse roue avant-gauche normalisée [-1, 1] → [-20, 20] rad/s
        - wheel_fr: vitesse roue avant-droite normalisée [-1, 1] → [-20, 20] rad/s
        - wheel_rl: vitesse roue arrière-gauche normalisée [-1, 1] → [-20, 20] rad/s
        - wheel_rr: vitesse roue arrière-droite normalisée [-1, 1] → [-20, 20] rad/s
        
    Note: Avec wheel_radius=0.15m, 20 rad/s = 3 m/s linéaire (vs 1 m/s max avec steering)
    """
    ...
    def _load_robot_params_from_xml(self):
        ...
        # CONTRÔLE DIRECT DES 4 ROUES : 4 actions (vitesses angulaires normalisées)
        # action[0] = wheel_fl (±1.0 → ±20 rad/s)
        # action[1] = wheel_fr (±1.0 → ±20 rad/s)
        # action[2] = wheel_rl (±1.0 → ±20 rad/s)
        # action[3] = wheel_rr (±1.0 → ±20 rad/s)
        self.action_space = spaces.Box(-1.0, 1.0, (4,), np.float32)
        self.max_wheel_speed = 20.0  # rad/s (équivalent à ~3 m/s linéaire avec r=0.15m)
    ...
    def step(self, action):
        ...
        if self.step_count < self.stabilization_steps:
            ...
            # CONTRÔLE DIRECT DES 4 ROUES : appliquer directement les vitesses normalisées
            action = np.clip(action, -1.0, 1.0)
            
            # Convertir actions normalisées [-1, 1] en vitesses angulaires [rad/s]
            w_fl = action[0] * self.max_wheel_speed
            w_fr = action[1] * self.max_wheel_speed
            w_rl = action[2] * self.max_wheel_speed
            w_rr = action[3] * self.max_wheel_speed
