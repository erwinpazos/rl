...
class RobotCorridorEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Apply action (clip to valid range and add forward bias)
        # simulation.py uses speed=5 for forward movement, so we bias actions toward that
        action = np.clip(action, -1.0, 1.0) * 10.0 + 5.0  # Scale to [-10, 10] + 5 = [-5, 15]
    ...
    def _compute_reward(self):
        ...
        if robot_x >= self.corridor_length:
            ...
            print(f"[TERM] FELL at step {self.current_step}: z={robot_z:.2f}")
        elif robot_x < -1.0:
            ...
            # Penalty for uneven terrain (bumps and holes)
            # Check if all 4 wheels are at the same height
            # Ignore first 50 steps to let robot stabilize after spawn
            if self.current_step > 50:
                ...
                for wheel_name in wheel_names:
                    wheel_body_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, wheel_name)
                    if wheel_body_id >= 0:
                        # Get wheel Z position from body xpos (world position)
                        wheel_z = self.data.xpos[wheel_body_id][2]
                        wheel_z_positions.append(wheel_z)
                
                if len(wheel_z_positions) == 4:
                    # Calculate variance in wheel heights
                    wheel_z_array = np.array(wheel_z_positions)
                    z_variance = np.var(wheel_z_array)  # Variance of wheel heights
                    
                    # CRITICAL: Terminate episode if robot is severely tilted (wheel in deep hole/high bump)
                    if z_variance > 0.01:  # Variance > 0.01 means ~10cm+ difference between wheels
                        reward = -50.0
                        terminated = True
                        info['termination_reason'] = 'unstable_terrain'
                        print(f"[TERM] UNSTABLE at step {self.current_step}: variance={z_variance:.4f}")
                        return reward, terminated, info
                    
                    # Penalty if wheels are not at same height (on bumps/holes)
                    # Lower threshold and stronger penalty to force flat terrain
                    elif z_variance > 0.0001:  # Strict threshold for flat ground
                        # Very strong penalty proportional to variance
                        penalty = z_variance * 500.0
                        reward -= penalty
