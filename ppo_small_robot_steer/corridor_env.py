...
def steer_angle_to_wheel_speeds(steering_angle, speed=0.5, 
    ...
    max_yaw_rate = np.radians(900.0)  # rad/s at max steering angle (vitesse optimale)
...
class CorridorEnv(gym.Env):
    ...
    def _is_colliding_with_bump(self):
        """Détecte si le robot est en collision avec un bump (pilier)."""
        ...
        for i in range(self.data.ncon):
            ...
            # Vérifier si une des géométries est un bump et l'autre fait partie du robot
            ...
            geom1_is_bump = geom1_name and 'bump' in geom1_name
            geom2_is_bump = geom2_name and 'bump' in geom2_name
            
            # Collision détectée si robot touche un bump
            if (geom1_is_robot and geom2_is_bump) or (geom2_is_robot and geom1_is_bump):
                return True
